#!/bin/bash -l
#SBATCH --job-name=SLURM_quality_filtering
#SBATCH --output=SLURM_quality_filtering_out.txt
...
# 2. BASEDIR: path to the base directory on the mounted server where adapter clipped fastq file are already stored in subdirectory titled "adapter_clipped/" and where the quality-filtered fastq files will be placed in a subdirectory titled "qual_filtered/" (ex: /workdir/cod/greenland-cod/). This is the same as the 3rd argument provided to the quality_filtering.sh script.
# 3. SAMPLELIST: path to a list of prefixes of the raw fastq files on the mounted server. This should be a subset of the the 1st column of the sample table (ex: /workdir/cod/greenland-cod/sample_lists/sample_list_pe_1.tsv). This is the same as the 1st argument provided to the quality_filtering.sh script.
...
SCRIPT=$PREFIX$ROOTDIR/data-processing/scripts/quality_filtering.sh
...
for SAMPLEFILE in `cat $TEMPORARY_SAMPLE_LIST`; do
  DATATYPE=`grep -P "${SAMPLEFILE}\t" $SAMPLE_TABLE_SLURM | cut -f 6`
  SAMPLE_ID=`grep -P "${SAMPLEFILE}\t" $SAMPLE_TABLE_SLURM | cut -f 4`
	SEQ_ID=`grep -P "${SAMPLEFILE}\t" $SAMPLE_TABLE_SLURM | cut -f 3`
	LANE_ID=`grep -P "${SAMPLEFILE}\t" $SAMPLE_TABLE_SLURM | cut -f 2`
	SAMPLE_SEQ_ID=$SAMPLE_ID'_'$SEQ_ID'_'$LANE_ID
	SAMPLEADAPT=$PREFIX$BASEDIR'adapter_clipped/'$SAMPLE_SEQ_ID
  if [ $DATATYPE = pe ]; then
...
if [ $FILTER = length ]; then
  bash $SCRIPT $TEMPORARY_SAMPLE_LIST $SAMPLE_TABLE_SLURM $BASEWORKDIR $FILTER $THREADS $FASTP $MAXLENGTH
else
  bash $SCRIPT $TEMPORARY_SAMPLE_LIST $SAMPLE_TABLE_SLURM $BASEWORKDIR $FILTER $THREADS $FASTP
fi

# Copy all output files back to the base directory on the mounted server
cp *_adapter_clipped_qual_filtered*.fastq.gz $PREFIX$BASEDIR'qual_filtered/'
